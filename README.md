...
- **三维沙盘 (`/cesium`)**
  - 多源图层：OSM 全球三维建筑、Cesium 世界地形、ArcGIS 卫星影像（替代原有失效的 Google 2D 瓦片源）、Google 写实 3D 场景、中国省界 GeoJSON（可开关）
  - 坐标系自动纠偏：省界 GeoJSON（GCJ-02）经 gcoord 一线库整体转换到 WGS-84，与 OSM 建筑（WGS-84）全球对齐
  - 图层校准：东西/南北偏移滑块实时平移矢量层对齐任意底图（按底图独立记忆），支持「GCJ-02 自动对齐」一键按视中心计算理论偏移
